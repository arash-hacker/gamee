...
//-----------------------------------------------------------------------------
//----------------------------------------------------------------------------- 
function gamee_gameInit() {
    gamee.gameInit("FullScreen", {}, [], function(error, data) {
        if(error !== null)
            throw error;
        
        // Registrovat callbacky
        gamee.emitter.addEventListener("start", function() {
            gamee_updateScore(0);
            gmCallback.gamee_onStart();
        });
        
        gamee.emitter.addEventListener("pause", function() {
            gmCallback.gamee_onPause();
        });
        
        gamee.emitter.addEventListener("resume", function() {
            gmCallback.gamee_onResume();
        });
        
        gamee.emitter.addEventListener("mute", function() {
            gmCallback.gamee_onMute();
        });
        
        gamee.emitter.addEventListener("unmute", function() {
            gmCallback.gamee_onUnmute();
        });
   
        // Oznamit dokonceni inicializace
        gmCallback.gamee_onInitialized(data.sound);
    });
}
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
function gamee_updateScore(score) {
