...
function gamee_init() {
    if (typeof gamee !== "undefined") {
        gamee.gameInit("FullScreen", {}, ["coins"], function (error, data) {
            if (!error) {
                // Callbacky
...
function gamee_purchaseItem(itemCost, itemName, itemImage) {
    if (gameeState < eGameeState.initialized)
        return false;
    ...
    gamee.purchaseItem({
        ...
        gmCallback.gamee_onItemPurchased(data && data.purchaseStatus);
    });    
