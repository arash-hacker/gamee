...
function gamee_init() {
    if (typeof gamee !== "undefined") {
        gamee.gameInit("FullScreen", {}, ["rewardedAds", "socialData"], function (error, data) {
            if (!error) {
                // Callbacky
...
function gamee_requestSocial() {
    gamee.requestSocial(function (error, data) {
        if(error !== null)
            throw error;
    
        if (data && data.socialData && data.socialData.friends) {
            var friends = data.socialData.friends;
            var numFriends = friends.length;
            
            gmCallback.gamee_onFriendListSize(numFriends);
            
            for (var i = 0; i < numFriends; i++) {
                var friend = friends[i];
                
                gmCallback.gamee_onFriendData(i, friend.name, friend.avatar, friend.highScore);
            }
        }
        else {
            gmCallback.gamee_onFriendListSize(-1);
        }
    });    
}
