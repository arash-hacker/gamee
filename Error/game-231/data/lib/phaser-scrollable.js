var ScrollableArea = function(game, x, y, w, h, heightAdjust, params) {
	params = params || {};
	
	Phaser.Group.call(this, game);

	//Added here for PoliceRunners purposes
		...
		if (this.startedInside) {
			...
			if (Math.abs(this.game.input.x-this.inputX) <= this.settings.clickXThreshold && Math.abs(this.game.input.y-this.inputY) <= this.settings.clickYThreshold) {
				for (var i=0;i<this.children.length;i++) {
					if (this.getChildAt(i).getBounds().contains(this.game.input.x, this.game.input.y) && this.getChildAt(i).inputEnabled && this.getChildAt(i).events.onInputUp.getNumListeners() > 0) {
...
ScrollableArea.prototype.limitMovement = function() {
	...
	if (this.settings.verticalScroll) {
		...
		if (this.y < -(this.height*this._heightAdjust-this._h-this._y)) {
			if (this.height*this._heightAdjust > this._h) {
				//console.log("2!");
				this.y = -(this.height*this._heightAdjust-this._h-this._y);
			}
			else {
				//console.log("3!");
				this.y = this._y;
